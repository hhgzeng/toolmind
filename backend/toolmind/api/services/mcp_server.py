...
class MCPService:
    ...
    async def get_all_servers(cls, user_id):
        try:
            ...
            for server in all_servers:
                user_config = await MCPUserConfigService.show_mcp_user_config(
                    user_id, server["mcp_server_id"]
                )
    ...
    async def mcp_server_need_update(cls):
        ...
        # 计算时间差
        time_difference = current_time - server.update_time.replace(
    ...
    async def get_mcp_server_ids_from_name(cls, mcp_servers_name, user_id):
        try:
            mcp_servers = await MCPServerDao.get_mcp_server_ids_from_name(
                mcp_servers_name, user_id
            )
            mcp_servers.extend(
                await MCPServerDao.get_mcp_server_ids_from_name(
                    mcp_servers_name, SystemUser
                )
            )
