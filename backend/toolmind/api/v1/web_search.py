...
def _update_web_search_api_key_in_yaml(api_key: str) -> None:
    """
...
async def get_web_search_config(login_user: UserPayload = Depends(get_login_user)):
    try:
        api_key = ""
        if getattr(app_settings, "tools", None) and getattr(app_settings.tools, "tavily", None):
            api_key = app_settings.tools.tavily.get("api_key") or ""

        return resp_200(data={"api_key": api_key})
...
async def update_web_search_config(req: WebSearchConfigReq, login_user: UserPayload = Depends(get_login_user)):
    try:
        ...
        # 同步更新到配置文件
        _update_web_search_api_key_in_yaml(req.api_key)

        return resp_200(data={"api_key": req.api_key})
    except Exception as e:
        return resp_500(message=str(e))
