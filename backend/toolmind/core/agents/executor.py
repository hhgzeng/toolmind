...
class Executor:
    """步骤执行 Agent：逐步执行子任务并产出结果"""

    def __init__(self, user_id: str, tool_manager: ToolManager):
        self.user_id = user_id
        self.tool_manager = tool_manager

    async def execute(self, state: MindState) -> AsyncGenerator[dict, None]:
        """
        逐步执行 state.steps 中的每个子任务。

        通过 yield SSE 事件向前端推送进度，执行完毕后 state.context_task 被填充。
        """
